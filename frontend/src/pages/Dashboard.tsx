...
const Dashboard = () => {
  ...
  const handleTerminalSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setTerminalLogs(prev => [...prev, { sender: 'system', text: `> Analyzing command: [${query.toUpperCase()}] ...` }]);
      setTimeout(() => {
        let response = '';
        if (query.toLowerCase().includes('status')) {
          response = 'TEE Enclave [OK]. All nodes active. Cryptographic signature verified: 0x' + Math.random().toString(16).substring(2, 10);
        } else if (query.toLowerCase().includes('analyze') || query.toLowerCase().includes('trade')) {
          response = 'Running Monte Carlo simulations... AI Agent consensus reached: BULLISH on ETH, holding steady on BTC.';
        } else {
          response = `Command recognized. Verifying intent via ZK-Proof... Access granted. EvoTrade AI is processing your request.`;
        }
        setTerminalLogs(prev => [...prev, { sender: 'ai', text: response }]);
      }, 1000);
    }, 400);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-50">
      ...
      {/* Header */}
      <header className="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 shadow-sm backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center gap-8 px-12">
            <div className="relative w-full max-w-md group">
              ...
              <input 
                ...
                className="w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-sm rounded-xl pl-10 pr-12 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/50 transition-all placeholder:text-slate-400"
              />
              <div className="absolute inset-y-0 right-2 flex items-center pointer-events-none">
                <span className="text-[10px] font-mono text-slate-400 border border-slate-200 dark:border-slate-800 rounded px-1.5 py-0.5 bg-white dark:bg-slate-900 shadow-sm">⌘K</span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="px-6 py-2 bg-white dark:bg-slate-900 text-slate-900 dark:text-white hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-slate-700 rounded-lg text-sm font-bold transition-colors flex items-center gap-2 group relative"
            >
      ...
      <div className="w-full bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 py-2 overflow-hidden relative flex items-center group">
            ...
            <div key={loopIndex} className="flex gap-12 px-6">
              {Object.entries(cryptoPrices).map(([id, data]: [string, any]) => (
                <div key={`${loopIndex}-${id}`} className="flex items-center gap-2 text-xs font-mono shrink-0">
                  <span className="text-slate-500 dark:text-slate-400 font-bold">{COIN_SYMBOLS[id]}</span>
                  <span className="text-slate-900 dark:text-white">${data?.usd?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 4 }) || '---'}</span>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 relative">
        ...
        <main className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-stretch relative z-10">
          ...
          {/* TEE Enclave Status */}
          <div className="p-6 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-sm border border-slate-200 dark:border-slate-800 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 relative overflow-hidden flex flex-col">
            <div className="absolute top-0 right-0 p-4 opacity-10">
              <Server className="w-24 h-24" />
            </div>
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-4 relative z-10">
            ...
            <div className="space-y-3 relative z-10">
              ...
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-slate-900 dark:text-white font-mono text-xs">AES-256-GCM</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-500 dark:text-slate-400">Uptime</span>
                <span className="text-slate-900 dark:text-white font-mono text-xs">99.99%</span>
              </div>
              <div className="flex justify-between items-center text-sm">
          ...
          <div className="p-6 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-sm border border-slate-200 dark:border-slate-800 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 flex flex-col">
            ...
            <div className="flex justify-center gap-4 text-xs font-semibold mt-2">
              {ASSET_ALLOCATION.map(asset => (
                <div key={asset.name} className="flex items-center gap-1">
                  ...
                  <span className="text-slate-600 dark:text-slate-400">{asset.name} {asset.value}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Execution History */}
          <div className="p-6 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-sm border border-slate-200 dark:border-slate-800 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 flex-1">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-4">
            ...
            <div className="space-y-4">
              {MOCK_EXECUTIONS.map((exec, i) => (
                <div key={i} className="flex flex-col gap-1 text-sm border-b border-slate-100 dark:border-slate-800 pb-3 last:border-0 last:pb-0">
                  <div className="text-slate-600 dark:text-slate-400 font-medium">{exec.action}</div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-500 dark:text-slate-400">{exec.time}</span>
          ...
          <div className="p-6 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-sm border border-slate-200 dark:border-slate-800 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 flex-1 flex flex-col">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-4">
            ...
            <div className="space-y-4 flex-1">
              {[
                ...
                { address: '0x8F2...3C9', power: '320k', color: 'text-slate-600 dark:text-slate-400' },
                { address: '0x3E1...7A5', power: '180k', color: 'text-orange-400' },
                { address: '0x9D4...2F1', power: '150k', color: 'text-slate-500 dark:text-slate-400' },
                { address: '0x2B8...1E9', power: '120k', color: 'text-slate-500 dark:text-slate-400' },
              ].map((user, i) => (
                <div key={i} className="flex items-center justify-between text-sm bg-slate-100 dark:bg-slate-800 p-2 rounded-lg">
                  <div className="flex items-center gap-3">
                    <span className={`font-bold ${user.color}`}>#{i + 1}</span>
                    <span className="text-slate-600 dark:text-slate-400 font-mono">{user.address}</span>
                  </div>
          ...
          {/* Treasury Stats */}
          <div className="p-6 rounded-2xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-md shadow-sm border border-slate-200 dark:border-slate-800 hover:-translate-y-1 hover:shadow-xl transition-all duration-300 relative overflow-hidden flex flex-col h-full">
            <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400 mb-4 relative z-10">
              <LineChart className="w-5 h-5" />
              <h2 className="font-semibold">Treasury TVL</h2>
            </div>
            <div className="text-4xl font-extrabold text-slate-900 dark:text-white mb-2 relative z-10">$1.24M</div>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-900 text-slate-100 border border-indigo-500/30 flex flex-col h-64 overflow-hidden shadow-[0_0_15px_rgba(99,102,241,0.1)]">
            <div className="bg-slate-50 dark:bg-slate-950/80 px-4 py-2 border-b border-indigo-500/20 flex items-center gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2 ml-2 pl-4 border-l border-slate-200 dark:border-slate-800">
                <button
                  ...
                  className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'active' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-slate-700'}`}
                >
                  Active
                </button>
                <button
                  onClick={() => setActiveTab('history')}
                  className={`px-4 py-2 text-sm font-semibold rounded-full transition-colors ${activeTab === 'history' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 dark:bg-slate-700'}`}
                >
                  History
                </button>
              </div>
          ...
          {activeTab === 'active' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 auto-rows-fr">
              {proposals.map((p) => {
                ...
                return (
                  <div key={p.id} className="p-5 rounded-2xl bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-950 transition-colors flex flex-col h-full">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2">
                        <span className="text-slate-500 dark:text-slate-400 text-sm font-mono">ID: #{p.id}</span>
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex-1 pr-4">{p.title}</h3>
            ...
            <div className="space-y-4">
              {voteHistory.length === 0 ? (
                <div className="p-8 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl text-slate-500 dark:text-slate-400">
                  <Activity className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                  <p>You haven't voted on any proposals yet.</p>
                </div>
              ) : (
                voteHistory.map((vote, idx) => (
                  <div key={idx} className="p-5 rounded-2xl bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                      <div className="text-sm text-slate-500 dark:text-slate-400 font-mono mb-1">ID: #{vote.id}</div>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white">{vote.title}</h3>
                    </div>
